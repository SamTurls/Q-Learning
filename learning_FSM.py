...
def parse_args():
    ...
    p.add_argument("--memory_length", type=int, default=2)
...
def apply_action_expected(x, y, o, prob):
    ...
    # Per-action next orientation
    o_action = o[:, None] + orientation[None, :]   # (N, A)

    # Per-action displacement
    dx = velocity[None, :] * jnp.cos(o_action) * dt
    dy = velocity[None, :] * jnp.sin(o_action) * dt

    # Expected next state
    x_next = x + jnp.sum(prob * dx, axis=1)
    y_next = y + jnp.sum(prob * dy, axis=1)

    # These are linear, so expectation is fine
    o_next = o + jnp.sum(prob * orientation[None, :], axis=1)
    v_next = jnp.sum(prob * velocity[None, :], axis=1)
...
def run_sim(key, N, T, QNN_params, temp):
    ...
    def step(carry, t):
        ...
        vis_state_integers = jnp.array(compute_visual_states(x, y, o))   # (N,)
        curr_vis_states = jax.vmap(lambda n: to_binary(n, n_s))(vis_state_integers)
        # jax.debug.print("{}", curr_vis_states)
...
#         actions = jnp.array([0 for i in range(5)])
        ...
        # roll agents so this agent is first
            ...
            vs = simulate_paths_vmap(paths, x_roll, y_roll, o_roll, velocity, orientation, QNN_params, prev_frames_roll, temp)
            #calculate entropy from visual states
            # jax.debug.print("{}", vs[0])
            e = entropy(vs[0],vs[1])
...
def train(key, online_params, target_params, opt_state, start, end, simulations, save_dir):
    ...
    for cycle in range(start, end):
        ...
        # if (cycle + 1) % 10 == 0 or cycle == start:
        ...
        t1 = time.time()
        tpc = (t1-t0)/(cycle+1)
        print ("Time remaining:", round(tpc * (simulations - cycle)/3600,2), " hours")

        print(f"\n=== Average Loss {jnp.mean(jnp.array(losses))} ===")
...
def main():
    global key, online_params, target_params, opt_state, discount_factor, N, memory_length
    args = parse_args()
    ...
    key, online_params, target_params, opt_state = train(
        key, online_params, target_params, opt_state, args.start, args.end, args.simulations, args.save_dir)
